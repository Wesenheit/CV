...
Working in the R\&D department on the machine learning solutions for the diagnostic of
the degradation of NXE litographic machines (various contaminations in the optical column).
\begin{itemize}
    \item Researching hybrid models integrating physical simulations with the machine learning models.
    Most of the work was centered around developing data-driven corrections to the physical model
    used in the diagnostic process.
    \item Based on the created model, implementing Bayesian inference algorithms to infer the contamination
    profile in the machine.
    \item High performance implementation of the inference algorithms including optimization
    for edge devices like oridnary laptops
    and large-scale HPC clusters.
\end{itemize}


{\sl Quantitative Developer intern, UBS AG \hfill July 2025 - September 2025} \\
Working on the high-performant, mulit-code C++ engine used to price loans within Lombard Lending unit.
\begin{itemize}
    \item Code benchmarking, bottleneck identification.
    \item Investigation of parallel efficiency of the code, load balancing of the code using IntelTBB.
    \item Memory and other optimizations.
    \item In the end, code after optimization was $3$ times faster and $2$ times more memory efficient.
\end{itemize}
